...
public class ResultActivity extends Activity {
    ...
    private void showImg() {
        ...
        for (int i = 0, j = images.size(); i < j - 1; i += 2) {
            ...
            Glide.with(this).load(new File(getImagePath(images.get(i)))).into(imageView1);
            Glide.with(this).load(new File(getImagePath(images.get(i + 1)))).into(imageView2);
            linearLayout.addView(view);
        }
        if (images.size() % 2 == 1) {
            View view = LayoutInflater.from(this).inflate(R.layout.image_show, null);
            ImageView imageView1 = (ImageView) view.findViewById(R.id.imgShow1);
            Glide.with(this).load(new File(getImagePath(images.get(images.size() - 1)))).into(imageView1);
            linearLayout.addView(view);
        }

    }

    private String getImagePath(TImage image) {
        if (image.isCompressed()) {
            return image.getCompressPath();
        } else {
